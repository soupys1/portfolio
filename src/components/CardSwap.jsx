  ...
  useEffect(() => {
    ...
    const setPositions = (animated = false) => {
      orderRef.current.forEach((cardIdx, stackPos) => {
        ...
        animated ? gsap.to(cards[cardIdx], { ...props, duration: 0.55, ease: 'power3.inOut' })
    ...
    const swap = () => {
      ...
      // Move remaining cards one step forward
      order.slice(1).forEach((cardIdx, i) => {
        gsap.to(cards[cardIdx], {
          ...
          ease:   'power3.inOut',
          overwrite: 'auto',
          onComplete: i === 0 ? () => gsap.to(cards[cardIdx], { skewY: 0, duration: 0.22, ease: 'power2.out' }) : undefined,
        });
      });
